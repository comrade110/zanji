...
//    //当设置过缓存开关后,如果缓存开关为0,则不加载图片缓存到沙盒中(注意:这里的沙盒不是指内存,只指确确实实的硬盘地址,本地位置!)
//    if([[NSUserDefaults standardUserDefaults]objectForKey:SAVEIMAGECACHE]&&
//       ![[NSUserDefaults standardUserDefaults]boolForKey:SAVEIMAGECACHE]){
//        
//        return nil;
//    }
    ...
    if (toDisk) {
        dispatch_async(self.ioQueue, ^{
            ...
            if (data) {
                ...
                BOOL b=[_fileManager createFileAtPath:[self defaultCachePathForKey:key] contents:data attributes:nil];
                //图片保存失败时,给出提示
                if(!b)
                    NSLog(@"save image to cache = %d",b);
            }
        });
    }
}
